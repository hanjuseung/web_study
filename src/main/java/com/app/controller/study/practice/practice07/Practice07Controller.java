...
public class Practice07Controller {
	...
	@GetMapping("/listTest")
	public String listTest(Model model,Member member,@RequestParam String type) {
//		//str 반복 케이스
//		// 1) 그냥 jsp 내에서 고정 텍스트 반복
//		
//		// 2) 전달할 스트링(메세지) 전달하고 그걸 반복
//		
//		// 3) 전달할 메세지를 리스트 형태로 전달 반복
//		
//		//화면 처리 케이스
//		// 1) type 으로 아예 화면 자체를 구분
//		/*
//		//type : str 혹은 member 둘중에 하나로 확정 전제
//		if(type.equals("str")) {
//			model.addAttribute("msg1","스트링 리스트입니다.");
//			
...
//		// 2) 같은 화면에서 type에 따라 동적으로 구분
	...
	//practice09번 문제 풀이를 위해 추가한 코드
	@GetMapping("/listTest/{type}")
	public String listTestAppend(Model model,@PathVariable String type,HttpServletRequest request) {
		if(type.equals("str")) {
			model.addAttribute("msg","스트링 리스트입니다.");
